...
const useDeckResult = () => {
    ...
    async function generateDeck(data: TFormValues, apiKey: string) {
        if (!data["target-language-select"] || !data["level-of-language-select"] || !data["native-language-select"] || !data["prompt-words-to-process"]) throw new Error("One of needed parameters is unefined")
